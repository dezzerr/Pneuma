...
export function filterBooks(query: string): string[] {
  ...
  return BOOK_NAMES.filter((name) => {
    ...
    const aStarts = aLower.startsWith(nq) || aLower.replace(/\s/g, "").startsWith(nq.replace(/\s/g, ""));
    const bStarts = bLower.startsWith(nq) || bLower.replace(/\s/g, "").startsWith(nq.replace(/\s/g, ""));
